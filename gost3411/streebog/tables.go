...
// pi — нелинейное биективное преобразование (ГОСТ Р 34.11-2012, 6.2).
// Совпадает с подстановкой "Кузнечика" из ГОСТ Р 34.12-2015.
var pi = [256]byte{
...
// tau — перестановка байт (ГОСТ Р 34.11-2012, 6.3): P(a)_i = a_tau(i).
// Используется эталонной реализацией в тестах; в рабочем коде свёрнута в
// таблицы lpsTable.
var tau = [64]byte{
...
// aMatrix — строки матрицы A линейного преобразования l
// (ГОСТ Р 34.11-2012, 6.4), в порядке возрастания номера строки.
...
// cHex — итерационные константы C[1]..C[12] (ГОСТ Р 34.11-2012, 6.5) в том
// же виде, в каком они напечатаны в стандарте: старший разряд слева.
// Разбор на слова выполняется в init, чтобы не переносить их вручную.
var cHex = [12]string{
...
// c — те же константы в машинном представлении: слово 0 хранит младшие
// 64 разряда вектора.
var c [12]state

// lpsTable сворачивает S, P и L в восемь таблиц по 256 значений:
//
//	lpsTable[k][x] = l(pi(x) << 8k)
//
// Тогда j-е слово результата LPS равно XOR восьми выборок, где из слова k
// берётся байт номер j. Всего 16 КиБ.
var lpsTable [8][256]uint64

// lRef вычисляет l(b) — умножение вектора на матрицу A над GF(2)
// (ГОСТ Р 34.11-2012, 6.4). Разряды нумеруются справа налево, поэтому
// старшему разряду b соответствует строка A с номером 0.
func lRef(b uint64) uint64 {
	var r uint64
	for i := 0; i < 64; i++ {
		if b&(1<<(63-uint(i))) != 0 {
			r ^= aMatrix[i]
...
// parseVector разбирает 128 шестнадцатеричных цифр стандарта в state:
// слово 0 — младшие 64 разряда, то есть последние восемь байт записи.
func parseVector(s string) state {
	raw, err := hex.DecodeString(s)
	if err != nil || len(raw) != BlockSize {
		panic("streebog: повреждённая константа в таблицах")
	}
	var v state
	for j := 0; j < 8; j++ {
		v[j] = binary.BigEndian.Uint64(raw[BlockSize-8*(j+1):])
	}
	return v
}
...
func init() {
	...
	for k := 0; k < 8; k++ {
		for x := 0; x < 256; x++ {
			lpsTable[k][x] = lRef(uint64(pi[x]) << (8 * uint(k)))
		}
	}
}
